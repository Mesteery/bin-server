...
const matchers = {
  js: [/<script.*src=["']([\w/.-]+)["']/gi, (file, from) => terser.minify({ [from]: file }).then(({ code }) => code)],
  css: [
    /<link.*(?:rel=["']?stylesheet["']?.*href=["']([\w/.-]+)["']|href=["']([\w/.-]+)["'].*rel=["']?stylesheet["']?).*/gi,
    (file, from) => cssm.process(file, { map: false, from }).then(({ css }) => css),
  ],
...
async function bundle([assetDirectory, htmlGlob], { gzip, svgo }) {
      ...
      for (const [, _match, match = _match] of file.matchAll(matcher[0])) {
        ...
        if (match in target) {
          ...
          try {
            ...
            target[match] = [await matcher[1](await readFile(from, 'utf8'), from), from, path];
          } catch (error) {
            console.error(error);
  ...
  for (const change in changes) {
    for (const path in changes[change]) {
      ...
      const to = `${ext === -1 ? from : from.slice(0, from.lastIndexOf(ext))}.${hash}${ext ? ext : ''}`;
      ...
      for (const htmlPath of htmlPaths) {
        ...
        htmlFiles[htmlPath][0] = htmlFiles[htmlPath][0].replace(
          ...
          `${ext === -1 ? path : path.slice(0, path.lastIndexOf(ext))}.${hash}${ext ? ext : ''}`,
        );
        changed = true;
      }
    }
...
const cli = meow(`
  Usage
	...
	Options
    --gzip, -g  Gzip level (1-9), 0 mean disabled, 0 by default
